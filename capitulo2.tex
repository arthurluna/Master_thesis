...
E usando a distância do ponto de incidência do raio na interface até o eixo ótico e a distância entre a interface e o plano focal na ausência da inteface (${\mathbf S}$ e ${\mathbf L_a}$ na figura\ref{ab_esf}), temos $\tan\theta=s/L_a$ e $\tan\theta_1=s/L$. Em ângulos pequenos, temos $\theta\approx s/L_a$ e $\theta_1\approx s/L$; e junto com a lei de Snell, temos:
%
\begin{equation}
...
Podemos entender esse efeito também como uma diferença de caminho ótico induzida no feixe. Ao serem refratadas, as componetes de onda-plana ganham também uma fase $e^{i\Psi(z, \theta)}$, onde

\begin{equation}
\Psi(z,\theta)=k\left( -\frac{L}{N^2}\cos\theta +(L+z)\cos\theta_1 \right) {\mathbf corrigir!!}
\label{faseSA}
\end{equation}
%
